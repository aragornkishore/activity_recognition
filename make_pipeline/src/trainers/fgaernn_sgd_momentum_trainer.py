...
class FGAE_SGDTrainer_Momentum(FGAE_SGDTrainer):
    ...
    def __init__(self, model, inputs, batchsize, loadsize, learningrate,
                 momentum=0.9, verbose=True, rng=None, recurrent_weight_scale=1.2):
        ...
        Args:
            ...
            recurrent_weight_scale: rescale weights after each batch,
                s.t. largest eigval equals this number
        """
        # momentum: annealing factor of the running avg. gradients
        self.recurrent_weight_scale = recurrent_weight_scale
        ...
        # rescale models recurrent weights to preferred scale
        self.model.rescale_recurrent_weights(
            np.float32(self.recurrent_weight_scale))
    ...
    def compile_train_functions(self):
        ...
        self._updateincs = theano.function(
            ...
            givens={self.model.inputs:
                    self.inputs_theano[:, bidx*self.batchsize:(bidx+1)*self.batchsize]
            })#, mode='DEBUG_MODE')
    ...
    def step(self):
        ...
        for load_idx in range(self.nloads):
            ...
            self.inputs_theano.set_value(
                self.inputs[load_idx * self.loadsize:
                            (load_idx + 1) * self.loadsize][indices].transpose(
                            1, 0, 2))
            ...
            for batch_idx in self.rng.permutation(self.nbatches):
                ...
                # rescale recurrent weights s.t. largest eigval is in preferred
                # scale
                self.model.rescale_recurrent_weights(
                    np.float32(self.recurrent_weight_scale))
        self.epochcount += 1
