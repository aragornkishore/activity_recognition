...
def sample_clips_dense(video, framesize, horizon,
    ...
    Args:
        ...
        stride: difference on an axis between direct neighbors
    """
    if type(video) == str:
    ...
    if stride:
        ...
        samples = np.zeros((nblocks_t * nblocks_w * nblocks_h,
                            framesize*framesize*horizon), dtype=np.uint8)
        idx = 0
...
def sample_clips_random(video, framesize, horizon, temporal_subsampling, nsamples):
    ...
    block_indices = [
        (np.random.randint(video.shape[0]-horizon),
         np.random.randint(video.shape[1]-framesize),
         np.random.randint(video.shape[2]-framesize)) for _ in range(
             int(nsamples))]
    return np.vstack([video[frame:frame + horizon,
                            row:row + framesize,
                            col:col + framesize].reshape(1, -1) for (
...
def get_num_subblocks(superblock_framesize, superblock_horizon, framesize, horizon, stride):
