...
#FIXME: hardcoded library location
this_path = os.path.realpath(
    os.path.abspath(os.path.split(inspect.getfile(
        inspect.currentframe() ))[0]))
libsvm = CDLL(os.path.join(this_path, '../libsvm.so.2'))
#if find_library('svm'):
...
#        libsvm = CDLL(os.path.join(this_path, '../windows/libsvm.dll'))
#    else:
#        libsvm_path = os.path.join(this_path, '../libsvm.so.2')
#        libsvm = CDLL(os.path.join(this_path, '../libsvm.so.2'))
